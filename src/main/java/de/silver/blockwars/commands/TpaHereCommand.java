    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("accept")) {
            if (tp.containsKey(p)) {
                ...
                p.sendMessage(Main.prefix + "Du hast die Teleport-Anfrage von §b" + tP.getName() + "§7 angenommen!");
                p.sendMessage(Main.prefix + "Du wirst in §b3 §7Sekunden Teleportiert. Bewege dich solange §cnicht§7!");
                p.playSound(p.getLocation(), Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1, 1);
                cooldown.add(p);
                tp.remove(p);
                Bukkit.getScheduler().runTaskLater(Main.getInstance(), () -> {
                    if (cooldown.contains(p)) {
                        p.sendMessage(Main.prefix + "Du wirst in §b2 §7Sekunden Teleportiert. Bewege dich solange §cnicht§7!");
                        p.playSound(p.getLocation(), Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1, 1);
                    }
                }, 20L);
                Bukkit.getScheduler().runTaskLater(Main.getInstance(), () -> {
                    if (cooldown.contains(p)) {
                        p.sendMessage(Main.prefix + "Du wirst in §b1 §7Sekunde Teleportiert. Bewege dich solange §cnicht§7!");
                        p.playSound(p.getLocation(), Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1, 1);
                    }
                }, 40L);
                Bukkit.getScheduler().runTaskLater(Main.getInstance(), () -> {
